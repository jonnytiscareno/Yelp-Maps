...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min(centroids, key= lambda elem: distance(location, elem))
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        closest_location = find_closest(restaurant_location(restaurant), centroids)
        centroid_pairs.append([closest_location,restaurant])
...
def find_centroid(cluster):
    ...
    y_long = []
    for elem in cluster:
        x_lat.append(restaurant_location(elem)[0])
        y_long.append(restaurant_location(elem)[1])
    return [mean(x_lat), mean(y_long)]
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(restaurants) for restaurants in cluster]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for feature in feature_fns:
        pred_func, rsquared = find_predictor(user, reviewed, feature)
        new_dict = {rsquared: pred_func}
        x.update(new_dict)
    highest_predictor = max(x.keys())
    return x[highest_predictor]
    # END Question 8
